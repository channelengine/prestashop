...
{
	private $id_channelEngine_order;
	...
	{
		return (boolean)Db::getInstance()->getValue('SELECT `id_channelEngine_order`
			FROM `'._DB_PREFIX_.'channelEngine_order`
	...
	public function getOrAddCustomer($channelEngine_profile)
	{
		$id_customer = (int)Db::getInstance()->getValue('SELECT `id_customer`
			FROM `'._DB_PREFIX_.'customer`
			WHERE `active` = 1
			AND `email` = \''.pSQL($this->email).'\'
			AND `id_shop` = '.(int)$channelEngine_profile->id_shop.'
		...
		if (!$id_customer)
		{
			...
			$customer->lastname = $format->formatName(ChannelEngineOrder::_formatFamilyName($this->familyname));
			$customer->firstname = $format->formatName($this->firstname);
			$customer->active = 1;
			$customer->id_shop = (int)$channelEngine_profile->id_shop;
			$res = $customer->add();
			
			$this->_writeLog($channelEngine_profile->id, 'add_customer', $res);
			
			$id_customer = $customer->id;
		}

		$this->id_customers[$channelEngine_profile->id_shop] = $id_customer;

		return $id_customer;
	}

	public function updateOrAddAddress($channelEngine_profile)
	{
		// Search if address exists
		$id_address = (int)Db::getInstance()->getValue('SELECT `id_address`
			FROM `'._DB_PREFIX_.'address`
			WHERE `id_customer` = '.(int)$this->id_customers[$channelEngine_profile->id_shop].'
			AND `alias` = \'eBay\'');
		...
		{
			$address = new Address();
			$address->id_customer = (int)$this->id_customers[$channelEngine_profile->id_shop];
		}
		...
		$address->alias = 'eBay';
		$address->lastname = $format->formatName(ChannelEngineOrder::_formatFamilyName($this->familyname));
		...
		$this->_writeLog($channelEngine_profile->id, 'add_address', $res, null, $is_update);
	...
	public function getProductsAndProfileByShop()
	{
		...
		foreach($this->product_list as $product) {
			if ($product['id_channelEngine_profile'])
				$channelEngine_profile = new ChannelEngineProfile((int)$product['id_channelEngine_profile']);
			else {
				$sql = 'SELECT epr.`id_channelEngine_profile`
				FROM `'._DB_PREFIX_.'channelEngine_product` epr
				WHERE epr.`id_product` = '.(int)$product['id_product'];
				$id_channelEngine_profile = (int)Db::getInstance()->getValue($sql);
				if ($id_channelEngine_profile) {
					$channelEngine_profile = new ChannelEngineProfile($id_channelEngine_profile);
				} else {
					// case where the row in channelEngine_product has disappeared like if the product has been removed from eBay before sync
					if (version_compare(_PS_VERSION_, '1.5', '>') && Shop::isFeatureActive()) {
						$sql = 'SELECT `id_channelEngine_profile`
							FROM `'._DB_PREFIX_.'channelEngine_profile` ep
						...
						$channelEngine_profile = ChannelEngineProfile::getCurrent();                        
					}
				}
			}
			
			if (!isset($res[$channelEngine_profile->id_shop])) {
				$res[$channelEngine_profile->id_shop] = array(
					'id_channelEngine_profiles' => array($channelEngine_profile->id),
					'id_products'      => array()
				);
			} elseif (!in_array($channelEngine_profile->id, $res[$channelEngine_profile->id_shop]['id_channelEngine_profiles']))
				$res[$channelEngine_profile->id_shop]['id_channelEngine_profiles'][] = $channelEngine_profile->id;
			
			// $res[$channelEngine_profile->id_shop]['id_products'][] = $product['id_product'];
			$res[$channelEngine_profile->id_shop]['id_products'][] = array('id_product' => $product['id_product'], 'id_product_attribute' => $product['id_product_attribute']);
		}
	...
	public function addCart($channelEngine_profile, $channelEngine_country)
	{
		$id_carrier = (int)ChannelEngineShipping::getPsCarrierByChannelEngineCarrier($channelEngine_profile->id, $this->shippingService);
		$cart = new Cart();

		$this->context->customer = new Customer($this->id_customers[$channelEngine_profile->id_shop]);
		
		$cart->id_shop = $channelEngine_profile->id_shop;
		$cart->id_customer = $this->id_customers[$channelEngine_profile->id_shop];
		...
		$cart->id_lang = $channelEngine_country->getIdLang();
		//$cart->id_currency = Currency::getIdByIsoCode($channelEngine_country->getCurrency());
		$cart->id_currency = $this->id_currency;
		$cart->recyclable = 0;
		$cart->gift = 0;
		$res = $cart->add();
		
		$this->_writeLog($channelEngine_profile->id, 'add_cart', $res, null);
		
		$this->carts[$channelEngine_profile->id_shop] = $cart;
	...
	public function updateCartQuantities($channelEngine_profile)
	{
		...
		if(isset($products_by_shop[$channelEngine_profile->id_shop]))
			$product_list = $products_by_shop[$channelEngine_profile->id_shop]['id_products'];
		else
			$product_list = array();
...
//			if (!count($this->carts[$channelEngine_profile->id_shop]->getProducts(false, $product['id_product'])))
//				continue;
			...
			if ($product['quantity'] >= $minimal_quantity)
			{
				...
				{
					$update = $this->carts[$channelEngine_profile->id_shop]->updateQty(
						...
						'up',
						0,
						new Shop($channelEngine_profile->id_shop));

					if ($update === true)
						$cart_nb_products++;
				}
				elseif ($this->carts[$channelEngine_profile->id_shop]->updateQty((int)$product['quantity'], (int)$product['id_product'], $id_product_attribute))
						$cart_nb_products++;
			}
		...
		$this->carts[$channelEngine_profile->id_shop]->update();

		if (version_compare(_PS_VERSION_, '1.5', '>'))
		{
			$this->carts[$channelEngine_profile->id_shop]->getProducts(true);
			$this->carts[$channelEngine_profile->id_shop]->getPackageList(true);
			$this->carts[$channelEngine_profile->id_shop]->getDeliveryOptionList(null, true);
		}

		return (boolean)$cart_nb_products;
	}

	public function validate($id_shop, $id_channelEngine_profile = null)
	{
		$customer = new Customer($this->id_customers[$id_shop]);
		$paiement = new ChannelEnginePayment();
		...
		$this->_writeLog($id_channelEngine_profile, 'validate_order', true, 'End of validate order');
	...
	public function updatePrice($channelEngine_profile)
	{
		...
		foreach ($this->product_list as $product)
		{
			...
			Db::getInstance()->autoExecute(
				...
				$detail_data, 
				'UPDATE', 
				'`id_order` = '.(int)$this->id_orders[$channelEngine_profile->id_shop].' AND `product_id` = '.(int)$product['id_product'].' AND `product_attribute_id` = '.(int)$product['id_product_attribute']
			);
		...
		if(version_compare(_PS_VERSION_, '1.5', '>')) 
		{
			$order = new Order((int)$this->id_orders[$channelEngine_profile->id_shop]);
			$data_old = $data;
			...
			Db::getInstance()->autoExecute(_DB_PREFIX_.'order_invoice', $invoice_data, 'UPDATE', '`id_order` = '.(int)$this->id_orders[$channelEngine_profile->id_shop]);
		...
		return Db::getInstance()->autoExecute(_DB_PREFIX_.'orders', $data, 'UPDATE', '`id_order` = '.(int)$this->id_orders[$channelEngine_profile->id_shop]);
	}
	...
	public function add($id_channelEngine_profile = null)
	{
		$this->id_channelEngine_order = ChannelEngineOrder::insert(array(
		...
		if(is_array($this->id_orders))
		{
			...
			{
				if (version_compare(_PS_VERSION_, '1.5', '>'))
					$res = Db::getInstance()->insert('channelEngine_order_order', array(
						'id_channelEngine_order' => (int)$this->id_channelEngine_order,
						'id_order'      => (int)$id_order,
						'id_shop'       => (int)$id_shop
					));
				else
					$res = Db::getInstance()->autoExecute(_DB_PREFIX_.'channelEngine_order_order', array(
						'id_channelEngine_order' => (int)$this->id_channelEngine_order,
						'id_order'      => (int)$id_order,
						'id_shop'       => (int)$id_shop
					), 'INSERT');
			...
			if($res)
				$this->_writeLog($id_channelEngine_profile, 'add_orders', $res);
		}
	}
	...
	private function _parseSku($sku, $id_product, $id_product_attribute, $id_channelEngine_profile)
	{
		...
		if (isset($data[2])) {
			...
			if (isset($data2[1]))
				$id_channelEngine_profile = (int)$data2[1];
		}

		return array($id_product, $id_product_attribute, $id_channelEngine_profile);
	}
	...
	private function _getProductsFromTransactions($transactions)
	{
		...
		foreach ($transactions as $transaction)
		{
			...
			$id_channelEngine_profile = 0;
			$quantity = (string)$transaction->QuantityPurchased;

			if (isset($transaction->Item->SKU))
				list($id_product, $id_product_attribute, $id_channelEngine_profile) = $this->_parseSku($transaction->Item->SKU, $id_product, $id_product_attribute, $id_channelEngine_profile);

			if (isset($transaction->Variation->SKU))
				list($id_product, $id_product_attribute, $id_channelEngine_profile) = $this->_parseSku($transaction->Variation->SKU, $id_product, $id_product_attribute, $id_channelEngine_profile);
			...
			if ($id_product)
				$products[] = array(
					...
					'id_channelEngine_profile' => $id_channelEngine_profile,
					'quantity' => $quantity,
	...
	private function _writeLog($id_channelEngine_profile, $type, $success, $response = null, $is_update = false)
	{
		if (!$this->write_logs)
			return;
		
		$log = new ChannelEngineOrderLog();
		$log->id_channelEngine_profile = (int)$id_channelEngine_profile;
		$log->id_channelEngine_order = (int)$this->id_channelEngine_order;
	...
	public static function getIdOrderRefByIdOrder($id_order)
	{
		return Db::getInstance()->getValue('SELECT eo.`id_order_ref` 
			FROM `'._DB_PREFIX_.'channelEngine_order` eo
			INNER JOIN `'._DB_PREFIX_.'channelEngine_order_order` eoo
			ON eo.`id_channelEngine_order` = eoo.`id_channelEngine_order`
			WHERE eoo.`id_order` = '.(int)$id_order);
	}

	public static function insert($data)
	{
		Db::getInstance()->autoExecute(_DB_PREFIX_.'channelEngine_order', $data, 'INSERT');
		return Db::getInstance()->Insert_ID();
	}

	public static function __set_state($attributes)
	{
		$channelEngine_order = new ChannelEngineOrder();
		
		foreach ($attributes as $name => $attribute)
			$channelEngine_order->{$name} = $attribute;

		return $channelEngine_order;
	}
}